...
class OptimizedPropertySearchEngine:
    ...
    def _prepare_optimized_indexes(self):
        ...
        self.price_to_sorted_idx = {int(row['price']): idx for idx, row in self.properties_df_sorted.iterrows()}
    ...
    def _get_price_range_indices(self, min_price: int, max_price: int) -> Tuple[int, int]:
        ...
        start_idx = int(np.searchsorted(prices, min_price, side='left'))
        end_idx = int(np.searchsorted(prices, max_price, side='right'))
